...
Console.WriteLine($"Skutečná délka textu: {text.Length}");
...
string blabol3 = "abbb";
string prvni;
if (string.Compare(blabol1, blabol2, StringComparison.Ordinal) < 0)
{
    if (string.Compare(blabol1, blabol3, StringComparison.Ordinal) < 0)
    {
        prvni = blabol1;
    }
    else
...
{
    if (string.Compare(blabol2, blabol3, StringComparison.Ordinal) < 0)
    {
        prvni = blabol2;
    }
    else
...
string veta = text.Substring(0, indexVykricniku + 1).Trim('!', '"');
...
int pocetAnd = text.ToLower().Split(new string[] { " and " }, StringSplitOptions.None).Length - 1;
